...
# Initialize GPIO for LED (fingerprint)
GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)
#GPIO.setup(18, GPIO.IN)

# Firebase setup
cred = credentials.Certificate("/home/Group22/Desktop/FaceRecognition/smart-security-system-417f0-firebase-adminsdk-fbsvc-ca47f4df54.json")
firebase_admin.initialize_app(cred, {"storageBucket": "smart-security-system-417f0.appspot.com"})
db = firestore.client()
...
# Function to capture and process fingerprint
def process_fingerprint():
    try:
        ...
        if positionNumber == -1:
            print('Fingerprint not recognized. Denied.')
            GPIO.setup(18, GPIO.IN) 
            return "Denied"
        else:
            print(f'Fingerprint recognized. Accepted. Position #{positionNumber}, Accuracy Score: {accuracyScore}')
        ...
        print('Fingerprint operation failed!')
        print('Exception message: ' + str(e))
        return "Denied"
...
def process_frame(frame):
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        if name != "Unknown":
            ...
            result = process_fingerprint()  # Call fingerprint scan after face recognition
            print(f"Fingerprint scan result: {result}")
            return frame
